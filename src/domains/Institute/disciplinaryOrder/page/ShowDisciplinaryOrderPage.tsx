...
const ShowDisciplinaryOrderPage = ({
  editable,
}: Props) => {
  ...
  const onSubmit = (data: any) => {
    ...
    if (selectedItems.length > 0)
      mutate({
        entity: `disciplinary-order/${id!=="new" ? "update" : "save"}`,
          ...
          <Grid container spacing={3} mt={1}>
            ...
            <Grid item xs={12} display="flex" justifyContent="flex-end" mt={2}>
              ...
              {editable && (
                <Button
                  variant="contained"
                  startIcon={id!=="new" ? <ChangeCircle /> : <AddCircle />}
                  type="submit"
                  disabled={isLoading}
                >
                  {isLoading
...
export default ShowDisciplinaryOrderPage;
