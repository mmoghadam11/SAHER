...
import { MoneyLundringMGM } from "../../forms/MoneyLundringMGM";
...
const AddMoneyLundringModal = ({
  ...
  const formItems: any[] = MoneyLundringMGM(setValue,{personnelOptions});
  useEffect(() => {
    if (editeData !== null) {
  ...
  const onSubmit = (data: FormData) => {
    mutate(
      ...
      },
      {
        ...
        onError: () => {
          snackbar("خطا در ایجاد مدیر", "error");
        },
      }
    );
  };
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Box display={"flex"} textAlign={"center"} alignItems={"center"}>
            ...
            <Typography variant="h6">
              {editeData ? `ویرایش مدیر انتخاب شده` : `مدیر مبارزه با پولشویی جدید`}
            </Typography>
          </Box>
...
export default AddMoneyLundringModal;
