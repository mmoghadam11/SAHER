...
const TerminateCooprationModal = ({
  ...
  const onSubmit = (data: FormData) => {
    mutate(
      {
        entity: `firm-partner/terminate-cooperation`,
      ...
      },
      {
        onSuccess: (res: any) => {
          ...
          if (!!editeData)
            snackbar(
              `اتمام شراکت با ${editeData?.personnelFirstName} ${editeData?.personnelLastName} با موفقیت انجام شد`,
              "success"
            );
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Box display={"flex"} textAlign={"center"} alignItems={"center"}>
            ...
            <Typography variant="body1">
              {editeData
                ? `درخواست اتمام شراکت با${editeData?.personnelFirstName} ${editeData?.personnelLastName}`
