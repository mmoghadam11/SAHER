...
const AccountantUserDisciplinaryOrderDetaile = (props: Props) => {
  const { id } = useParams();
  const Auth = useAuth();
      ...
      >
        <Grid item display={"flex"}>
          ...
          <Typography variant={isMobile ? "body1" : "h5"}>
            احکام انتظامی {Auth?.userInfo?.firstName + " " + Auth?.userInfo?.lastName}
          </Typography>
        </Grid>
        <Grid item display={"flex"}>
...
export default AccountantUserDisciplinaryOrderDetaile;
