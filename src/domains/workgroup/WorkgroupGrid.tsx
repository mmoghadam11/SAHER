...
const WorkgroupGrid = (props: Props) => {
  const Auth = useAuth();
  ...
  const {
    ...
    queryKey: [`workgroup/search${paramsSerializer(filters)}`],
  ...
  const columns: GridColDef[] = [
    { field: "name", headerName: "نام کارگروه", flex: 2 },
    { field: "workgroupSpecialityName", headerName: "زمینه تخصصی", flex: 1 },
    {
      headerName: "عملیات",
      field: "action",
      flex: 1,
    ...
    <Grid container justifyContent="center">
      ...
      >
        <Box display={"flex"}>
          ...
          <Typography variant="h5">کارگروه جامعه</Typography>
        </Box>
      ...
      <ConfirmBox
        ...
        handleSubmit={() =>
          mutate(
            {
              entity: `workgroup/delete/${deleteData?.id}`,
              method: "delete",
            },
            {
              onSuccess: (res: any) => {
                snackbar(`کارگروه انتخاب شده با موفقیت حذف شد`, "success");
                StatesData_refetch();
              },
              onError: () => {
        ...
        message={`آیا از حذف کارگروه ${deleteData?.name} مطمعین میباشید؟`}
        title={"درخواست حذف!"}
      />
    </Grid>
  );
