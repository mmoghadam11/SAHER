...
import { TerminateFormItem } from "domains/firmAdmin/staff/components/TerminateFormItem";
...
const TerminateCooprationModal = ({
  ...
  const onSubmit = (data: any) => {
    mutate(
      {
        entity: `money-laundering/terminate-cooperation?endDate=${data?.endDate}&id=${editeData.id}`,
      ...
      },
      {
        onSuccess: (res: any) => {
          ...
          if (!!editeData)
            snackbar(
              `اتمام همکاری با مدیر مبارزه با پولشویی انجام شد`,
              "success"
            );
          else snackbar(`اتمام همکاری با مدیر مبارزه با پولشویی انجام شد`, "success");
          refetch();
          //   handleClose();
        },
        onError: () => {
