...
const EDUGrid = ({}: Props) => {
  ...
  const {
    ...
    queryKey: [`educational-degree/search${paramsSerializer(filters)}`],
  ...
  const columns: GridColDef[] = [
    { field: "cdEducationalFieldName", headerName: "رشته", flex: 1 },
    { field: "cdUniversityName", headerName: "دانشگاه", flex: 1 },
    { field: "cdEducationalCertificateName", headerName: "مدرک", flex: 1 },
    { field: "description", headerName: "توضیحات", flex: 1 },
    {
      field: "educationCertificateDate",
    ...
    <Grid container item md={12} justifyContent="center">
      ...
      <ConfirmBox
        ...
        handleSubmit={() =>
          mutate(
            {
              entity: `educational-degree/delete/${deleteData?.id}`,
              method: "delete",
            },
            {
              onSuccess: (res: any) => {
                snackbar(`سابقه تحصیلی انتخاب شده با موفقیت حذف شد`, "success");
                StatesData_refetch();
              },
              onError: () => {
        ...
        message={`آیا از حذف سابقه تحصیلی مورد نظر مطمعین میباشید؟`}
        title={"درخواست حذف!"}
      />
    </Grid>
  );
