import { Article, Groups, Search, Settings } from "@mui/icons-material";
import {
  Box,
  Button,
  Dialog,
...
import AddCAOrganization from "./AddCAOrganization";

type Props = {};

const CAOrganizationGrid = (props: Props) => {
  const Auth = useAuth();
  ...
  const {
    ...
    queryKey: [`auditing-organization/search${paramsSerializer(filters)}`],
  ...
  const columns: GridColDef[] = [
    {
      field: "certifiedAccountantFirstName",
      headerName: "حسابدار رسمی",
      flex: 2,
    },
    // { field: "certifiedAccountantnationalCode", headerName: "کد ملی", flex: 2 },
    {
      field: "startDate",
    ...
    <Grid container justifyContent="center">
      ...
      >
        <Box display={"flex"} gap={1}>
          ...
          <Typography variant="h5">شاغلین سازمان حسابرسی</Typography>
        </Box>
      ...
      <ConfirmBox
        ...
        handleSubmit={() =>
          mutate(
            {
              entity: `auditing-organization/delete/${deleteData?.id}`,
              method: "delete",
            },
            {
              onSuccess: (res: any) => {
                snackbar(`شاغل سازمان حسابرسی انتخاب شده با موفقیت حذف شد`, "success");
                StatesData_refetch();
              },
              onError: () => {
        ...
        message={`آیا از حذف شاغل سازمان حسابرسی مورد نظر مطمعین میباشید؟`}
        title={"درخواست حذف!"}
      />
    </Grid>
  );
...
export default CAOrganizationGrid;
